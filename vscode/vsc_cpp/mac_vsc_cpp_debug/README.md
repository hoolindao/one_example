...
path 使用 `${file}` `${fileDirname}/` `${fileBasenameNoExtension}` 这些参数指具体在哪个文件执行 `Tasks: Run Build Task`

比如在 main.cpp 执行，会在预设文件根目录生成
...
如果想每次都编译指定 cpp 文件可使用 `${workspaceFolder}`

```json
            "args": [
                "${workspaceFolder}/main.cpp", // 指定编译文件
                "-o", // 指定输出文件名，不加该参数则默认输出a.exe，Linux下默认a.out
...
- 使用 vscode 内建 terminals 或 调用系统 terminals

## 解决，“能正常编译 hello world 程序，但 vscode 编辑器报错无法 include 头文件”

- 命令行执行以下命令搜索 c++ incloude 文件夹路径

```shell
gcc -v -E -x c++ -
```

- 添加相关路径到 `c_cpp_properties.json`
